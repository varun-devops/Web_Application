...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['std_id'])) {
        ...
        $sql = "SELECT * FROM student_table WHERE Std_Id = '$std_id'";
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            if ($vehicle_type === "Two-wheeler") {
                header('Location: twostd.php?std_id=' . $std_id);
            } elseif ($vehicle_type === "Four-wheeler") {
                header('Location: fourstd.php?std_id=' . $std_id);
            } else {
...
<div class="purpose">
    ...
    <button onclick="redirectToRegistration('Student')">Register</button>
</div>
